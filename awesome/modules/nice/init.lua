...
-- Shapes
local shapes = require("modules.nice.shapes")
...
_private.floating_color = "#4CBB17"
...
-- ============================================================
local table = table
local t = require("modules.nice.table")
table.save = t.save
table.load = t.load
...
local function create_button_image(name, is_focused, event, is_on)
    ...
    if not _private[key_color] then
        ...
        if not is_focused and event ~= "hover" then
            button_color = colors.hsv2hex(H, 0, 30)
        end
...
local function get_titlebar_mouse_bindings(c)
    ...
    if _private.win_shade_enabled then
        buttons[#buttons + 1] = abutton({}, _private.mb_win_shade_rollup,
                                        function()
            _private.shade_roll_up(c)
        end)
        buttons[#buttons + 1] = abutton({}, _private.mb_win_shade_rolldown,
                                        function()
            _private.shade_roll_down(c)
        end)
    end
...
function _private.add_window_decorations(c)
    ...
    local stroke_color_inner_bottom = lighten(lighten_amount *
    ...
    local corner_bottom_left_img = shapes.flip(
            ...
            stroke_offset_inner = 1.5,
            stroke_offset_outer = 0.5,
            stroke_source_outer = gradient(stroke_color_outer_bottom,
                                           stroke_color_outer_sides,
                                           bottom_edge_height, 0, 0.25),
            stroke_source_inner = gradient(stroke_color_inner_bottom,
                                           stroke_color_inner_sides,
                                           bottom_edge_height),
            stroke_width_inner = 1.5,
